...
public:
	...
	void whateverDeleteTail(){						//an extra function used to delete from tail, had to implement to make Undo function work
		node *temp = head;
		if (head == NULL)
		{
	...
	void printOnePage(int pageGiven){						//function used to print only one page, only 10 or if there are less than 10 lines, it'll print only those lines
		node *temp = head;
		if (numOfLines < pageGiven*10)   //25<30
		{
			int iterator = 1;
		...
		{
			cout<<"you want to print an inexisting page, collect yourself"<<endl;
		}
	}
